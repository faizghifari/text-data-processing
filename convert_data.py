...
def tokenize(text):
    ...
    request_result = requests.post(
        request_url_tokenize, json=data, headers=headers).json()
...
def get_subjectivity(text):
    response = requests.post(
        'http://10.181.131.244:4445/subjectivity', json={"texts": [text]})
...
def convert(lines, texts, split_texts, convert_type="sent"):
    annotations = []

    for index, line in enumerate(lines):
        if texts[index].strip():
            if convert_type == "sent":
                list_concepts = get_concepts(line, split_texts[index])
                annotations.append({
                    "sentence": texts[index],
                    "list of concept": list_concepts
                })
            elif convert_type == "sug":
                annotations.append({
                    "sentence": texts[index],
                    "target": [t["name"] for t in specific_info(line, "target")],
                    "mark": [s["name"] for s in specific_info(line, "suggestion")],
                    "information": [],
                    "object": [o["name"] for o in specific_info(line, "object")],
                    "reason": [r["name"] for r in specific_info(line, "reason")]
                })
...
def write_raw_xls(base_output, filename):
    ...
    xls_writer = pd.ExcelWriter(os.path.join(  # pylint: disable=abstract-class-instantiated
...
if __name__ == "__main__":
    ...
    # files = glob.glob('data/suggestion/aspect/kemendikbud/*.tsv')

    # texts, lines, split_texts = read_files(files)
    # raw_results = convert(lines, texts, split_texts, convert_type="sug")
    ...
    #         r['id'] = 0
    # print(f'NOT FOUND TOTAL : {not_found} from {len(raw_results)}')
    ...
    for chunk in chunks:
        ...
        response = requests.post(
            'http://cxa.prosa.ai/upload_data/', json=results)
